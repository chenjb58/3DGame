...
public class InteracteGUI : MonoBehaviour {
    UserAction UserAcotionController;           //管理玩家动作
    public int SetState { get { return GameState; } set { GameState = value; } }    
    static int GameState = 0;       //游戏状态 0为正在进行 1为失败 2为获胜
...
public class ClickGUI : MonoBehaviour{
    UserAction UserAcotionController;       //实现UserAction接口
    GameObjects GameObjectsInScene;         //被点击的对象
    ...
    void OnMouseDown()
    {
        if (gameObject.name == "boat")
        {
            ...
            UserAcotionController.MoveBoat();       //点击的是船,移动船
        }
        else
        {
            Debug.Log("Click_item!");
            UserAcotionController.ObjectIsClicked(GameObjectsInScene);      //点击的是人物,移动人物
        }
    }
}